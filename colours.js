...
async function colours()
{
    ...
    for(let i=0;i<4;i++)    //loop goes thru all 4 blocks
    {
        let randomIndex = Math.floor(Math.random() * blocks.length);    //randomize blocks
        let randomBlock = blocks.splice(randomIndex, 1)[0]; //adress the block and then delete it
        let randomBlinks = Math.ceil(Math.random()*level_value);    //random amount of blinks

        for(let j=0;j<randomBlinks;j++) //loop makes blocks blink
        {
            randomBlock.style.boxShadow = `0 0 10px 10px ${randomBlock.style.backgroundColor}`; //display shadow
            await delay(500);   //wait
            randomBlock.style.boxShadow = "none";   //hide shadow
            await delay(500);   //wait
        }
        required_count[i] = randomBlinks;   //adress required amount of clicks
    }
    answer();   //function that will handle counting and checking users clicks
}
